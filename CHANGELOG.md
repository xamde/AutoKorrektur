...
### Changed
- `RELEASE_CHECKLIST.md` brought up to date: items long since done (keystore, signing config, privacy policy hosting, licence notices, locale completeness, descriptions, hi-res icon) ticked with the facts that settle them; the still-open ones narrowed to what is actually left (tag the release, feature graphic, `core` screenshots, Data Safety form, build/upload) and the asset fetch added as a build prerequisite.
- PR #8 ("Introduce ViewModel and Coroutines", Nov 2025) closed as superseded: `main` has had a `viewModelScope`/`StateFlow` `MainViewModel` for a while, the branch's `LiveData` variant would have been a second one, and its other change (dropping the downshift slider) conflicts with that slider now being a deliberate debug-only evaluation control. Its branch and the merged `refactor-replace-yolo-inference-tflite` branch were deleted.

...
- `docs/MVP_FEATURE_FLAG_PLAN.md`: proposal for a build-time feature-flag tier system (`core`/`plus`/`beta`/`full`) to ship a radically simplified MVP flavor while keeping cut features re-enterable. Migration steps 1–3 implemented: see Changed/Removed below.
- Per-tier `BuildConfig` feature flags (`FEATURE_LIVE_AR`, `FEATURE_VIDEO_SNIPPETS`, `FEATURE_CLOUD_SDXL`, `FEATURE_HIGH_RES_PROGRESSIVE`, `FEATURE_MANUAL_MASK_BRUSH`, `FEATURE_BATCH_PROCESSING`, `FEATURE_EXTRA_EXPORT_LAYOUTS`, `FEATURE_EVALUATION_MODE`), all currently hardcoded `true` (`FEATURE_EVALUATION_MODE`: debug-only). `FirstFragment.applyFeatureFlags()` and `InstagramExportBottomSheet.applyFeatureFlags()` now gate their respective UI entry points on these flags — zero behavior change today since every flag still evaluates `true` (verified via `testDebugUnitTest` and an on-device screenshot); real per-tier differences arrive with the product flavors in a later step.

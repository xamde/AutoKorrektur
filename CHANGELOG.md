...
### Removed
- `TODO-for-human.md` (owner-only device/testing notes) and `BRANDING.md` (design decision doc) are no longer in the repo — gitignored, kept locally. The former was also purged from history along with every large binary: `git filter-repo` removed the OpenCV `.so`s, the models, the test fixtures, the old `AndroidKorrektur/` binaries and the generated benchmark report from all 224 commits, taking the repo from **668 MB to 13 MB**. HEAD's tree is byte-identical to before the rewrite; every branch and tag was rewritten and force-pushed, so **existing clones and forks must re-clone** (`git fetch` will not reconcile).

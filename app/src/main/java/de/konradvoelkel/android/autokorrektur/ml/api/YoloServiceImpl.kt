...
class YoloServiceImpl(
    ...
    override fun initialize(modelName: String, useFP16: Boolean) {
    ...
    override fun infer(
        ...
        // 2) Parse detections from raw detection buffer
        // Expected detShape = [1, features, proposals]
        ...
        try {
            ...
            // If ratios imply letterboxing was applied, crop to the content region
            if (contentW != shapes.inputW || contentH != shapes.inputH) {
                val roi = Rect(0, 0, contentW, contentH)
                val cropped = Mat(result, roi).clone()
                if (result !== cropped) result.release()
                result = cropped
            }
            ...
            if (hasOriginal) {
                ...
                if (result !== resized) result.release()
                result = resized
            }
        } catch (e: Exception) {

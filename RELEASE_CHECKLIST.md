...
- [x] `versionCode` / `versionName` are computed from git (`git rev-list --count HEAD`, `git describe --tags`) — nothing to edit; tag the release (`git tag -a v1.0.0`) so the name is clean.
